...
func (w *Writer) ReadLast(n int) []collector.Alarm {
	if !w.cfg.Enabled || w.cfg.FilePath == "" {
		return nil
	}

	f, err := os.Open(w.cfg.FilePath)
	if err != nil {
		return nil
	}
	defer f.Close()
	...
	alarms := make([]collector.Alarm, len(records))
	for i, r := range records {
		ts, _ := time.Parse(time.RFC3339, r.Timestamp)
		alarms[len(records)-1-i] = collector.Alarm{
